...
window.showSuccess = function(message) {
  window.paymentFlowMessageEl.classList.add('alert-success');
  window.paymentFlowMessageEl.classList.remove('alert-danger');
  window.paymentFlowMessageEl.innerText = message;
}

window.showError = function(message) {
  window.paymentFlowMessageEl.classList.add('alert-danger');
  window.paymentFlowMessageEl.classList.remove('alert-success');
  window.paymentFlowMessageEl.innerText = message;
}
    ...
    window.createPayment = async function (token) {
        ...
        try {
            $.ajax({
                url: 'process-payment',
                data: { Token: token, Amount: window.amount, InvoiceIds: window.invoiceIds, AmountsToPay: window.AmountsToPay },
                async: false,
                cache: false,
                success: function (data) {
                    if (data.errors && data.errors.length > 0) {
                        ...
                        window.location.href = "/Open-Invoices";
                    }
                },
