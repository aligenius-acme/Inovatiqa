...
var Checkout = {
    ...
    setStepResponse: function (response) {
        if (response.update_section) {
            $('#cosmic-' + response.update_section.name).hide();
...
var Billing = {
    ...
    nextStep: function (response) {
        ...
        if (response.update_section.newAddressId.length > 0) {
            const elem = document.createElement('option');
            elem.innerText = response.update_section.newAddress;
            elem.value = response.update_section.newAddressId;
            elem.setAttribute('selected', 'selected');
            document.getElementById('billing-address-select').appendChild(elem);
        }
...
var Shipping = {
    ...
    nextStep: function (response) {
        ...
        if (response.update_section.newAddressId.length > 0) {
            ...
            elem1.innerText = response.update_section.newAddress;
            elem1.value = response.update_section.newAddressId;
            elem1.setAttribute('selected', 'selected');
            document.getElementById('shipping-address-select').appendChild(elem1);
        }
        Checkout.setStepResponse(response);
    },
...
var ShippingMethod = {
    ...
    save: function () {
        ...
        if (this.validate()) {
            ...
            const ShippingFeeValue = $('input[name=shippingoption]:checked').attr('shippingfee').replace('$', '');
            ShippingFee.text(ShippingFeeValue);

            $('#SubTotal').text((parseFloat($('#order-total').text()) + parseFloat(ShippingFeeValue)).toFixed(2));
...
var PaymentInfo = {
    ...
    save: function () {
        ...
        //const elem = document.createElement('option');
        //elem.innerText = 'This is new Address';
        //elem.value = '1234';
        //document.getElementById('billing-address-select').appendChild(elem);
    },
...
var ConfirmOrder = {
    ...
    save: function () {
        if (!$('#PrivatePolicy').is(':checked') && !$('#terms-Condition').is(':checked')) {
            $('#checkbox-border').css("border", "1px solid red");
            $('#checkbox-error').show();
            $('#checkbox-border1').css("border", "1px solid red");
            $('#checkbox-error1').show();
            return;
        }
        var postData = {
            message: $('#AddOrderNoteMessage').val(),
            downloadId: 0
        };
        if (Checkout.loadWaiting !== false) return;
