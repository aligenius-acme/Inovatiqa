...
namespace Inovatiqa.Services.Orders
{
    public partial class OrderReportService : IOrderReportService
    {
        ...
        public OrderReportService(IDateTimeHelperService dateTimeHelper,
        ...
        {
            var query_tmp = (from orderItem in _orderItemRepository.Query()
                             join o in _orderRepository.Query() on orderItem.OrderId equals o.Id
            ...
            var query = from p in _productRepository.Query()
                        join pm in _productManufacturerRepository.Query() on p.Id
                            equals pm.ProductId
                            into p_pm
                        from pm in p_pm.DefaultIfEmpty()
                        join pc in _productCategoryRepository.Query() on p.Id
                            equals pc.ProductId
                            into p_pc
            ...
            if (storeId > 0)
            {
                query = from p in query
                        join sm in _storeMappingRepository.Query()
            ...
            bool showHidden = false)
        {
            ...
            var query1 = from orderItem in _orderItemRepository.Query()
                         join o in _orderRepository.Query() on orderItem.OrderId equals o.Id
            ...
            if (categoryId > 0)
            {
                query1 = from orderItem in query1
                         join p in _productRepository.Query() on orderItem.ProductId equals p.Id
                         join pc in _productCategoryRepository.Query() on p.Id equals pc.ProductId
                            into p_pc
                         from pc in p_pc.DefaultIfEmpty()
            ...
            if (manufacturerId > 0)
            {
                query1 = from orderItem in query1
                         join p in _productRepository.Query() on orderItem.ProductId equals p.Id
                         join pm in _productManufacturerRepository.Query() on p.Id equals pm.ProductId
                            into p_pm
                         from pm in p_pm.DefaultIfEmpty()
            ...
            var query2 =
                ...
                {
                    ProductId = g.Key,
                    TotalAmount = g.Sum(x => x.PriceInclTax),
            ...
            PaymentStatus? ps, ShippingStatus? ss, DateTime? startTimeUtc, DateTime? endTimeUtc)
        {
            ...
            var report = (from oq in query
                          join a in _addressRepository.Query() on oq.BillingAddressId equals a.Id
            ...
            string billingPhone = null, string billingEmail = null, string billingLastName = "", string orderNotes = null)
        {
            ...
            if (!string.IsNullOrEmpty(orderNotes))
                query = from o in query
                        join n in _orderNoteRepository.Query() on o.Id equals n.OrderId
            ...
            string billingPhone = null, string billingEmail = null, string billingLastName = "", string orderNotes = null)
        {
            ...
            var query = from orderItem in _orderItemRepository.Query()
                        ...
                        where (storeId == 0 || storeId == o.StoreId) &&
                              ...
                              (warehouseId == 0 ||
                                  ...
                                  _productWarehouseInventoryRepository.Query().Any(pwi =>
                              ...
                              (dontSearchOrderNotes || _orderNoteRepository.Query().Any(oNote =>
