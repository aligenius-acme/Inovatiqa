...
namespace Inovatiqa.Services.Customers
{
    public partial class CustomerReportService : ICustomerReportService
    {
            ...
            int pageIndex = 0, int pageSize = 214748364)
        {
            ...
            var query1 = from c in _customerRepository.Query()
                         join o in _orderRepository.Query() on c.Id equals o.CustomerId
        ...
        {
            var date = _dateTimeHelperService.ConvertToUserTime(DateTime.Now).AddDays(-days);

            var registeredCustomerRole = _customerService.GetCustomerRoleBySystemName(InovatiqaDefaults.RegisteredRoleName);
